...
onUpdated(() => {
  const hiddenToasts = props.msgs.filter((obj) => {
  ...
  hiddenToasts.forEach(function (error) {
    ...
    errorToast.addEventListener('hidden.bs.toast', function () {
      const indexOfObject = props.msgs.findIndex((item) => {
        return item.id === error.id;
      });
      if (indexOfObject !== -1) {
        props.msgs.splice(indexOfObject, 1);
      }
    });
  });
});
...
<template>
  ...
  >
    <div
      v-for="item in msgs"
      v-bind:id="item.id"
      class="toast fade opacity-75 bg-success"
      role="alert"
      aria-live="assertive"
      aria-atomic="true"
      data-bs-delay="15000"
